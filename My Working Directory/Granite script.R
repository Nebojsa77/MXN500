...
ldeaths.long <- gather(ldeaths, key=Month, value = Deaths,
                       -Year)
ldeaths.year <- group_by(ldeaths.long, Year)
summarise(ldeaths.year, annual.deaths = sum(Deaths))
ldeaths.month <- group_by(ldeaths.long, Month)
summarise(ldeaths.month, monthly.avg = mean(Deaths))
